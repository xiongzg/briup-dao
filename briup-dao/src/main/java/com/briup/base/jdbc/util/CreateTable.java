...
public class CreateTable {
		...
		try {
			...
			if(directory){
				...
				for(File childFile : listFiles){
					...
					if(append){
						//查询外键的sql
						String fksql = SqlStaticData.createTableConstraintSql(tableName);
						//String fksql = "select CONSTRAINT_NAME from user_constraints e where CONSTRAINT_TYPE!='P' and upper(e.table_name) = '"+tableName.toUpperCase()+"'";
						
						//System.out.println("查询外键的sql 测试："+testfksql);
						//System.out.println("查询外键的sql 成功："+fksql);
						
						ResultSet fkset = statement.executeQuery(fksql);
						while(fkset.next()){
							String constraint_name = fkset.getString("CONSTRAINT_NAME");
							//System.out.println(fksql+"  查询到的外键关系 111 : " +constraint_name);
							String deleteContraintSql = SqlStaticData.createDropConstraintSql(tableName, constraint_name);
							//String deleteContraintSql = "alter table "+tableName+"  drop constraint "+constraint_name;
							
							//System.out.println("开始 删除外键的sql 测试："+testdeleteContraintSql);
							//System.out.println("开始 删除外键的sql 成功："+deleteContraintSql);
					...
					for(Field field:declaredFields){//变量pojo类的属性
						...
						if (!field.isAnnotationPresent(TempField.class) && !field.isAnnotationPresent(Join.class)) {
							//列名
			    			...
			    			if(field.isAnnotationPresent(OneToOne.class)){
			    				...
			    				String alterAddForeign = SqlStaticData.createAddForeignKeySql(tableName, columnName, referenceTableName, "id");
			    				/*String alterAddForeign = "alter table ${table} add constraint ${constraint} foreign key(${forekey}) references ${referenceTable}(id)";
			    				if(onDeleteCascade){
			    					alterAddForeign +=" on delete cascade";
			    				}else if(onDeleteSetNull){
			    					alterAddForeign +=" on delete set null";
			    				}else{}
			    				//替换表名
			    				alterAddForeign =alterAddForeign.replace("${table}", tableName);
			    				//替换约束名
			    				alterAddForeign =alterAddForeign.replace("${constraint}", tableName+"_"+columnName+"_fk");
			    				//替换外键列名
			    				alterAddForeign =alterAddForeign.replace("${forekey}", columnName);
			    				//替换引用表的名字
			    				alterAddForeign =alterAddForeign.replace("${referenceTable}", referenceTableName);*/

			    				
			    				//System.out.println("一对一 添加外键的sql 测试："+testalterAddForeign);
								//System.out.println("一对一 添加外键的sql 成功："+alterAddForeign);
			    				...
			    				String fkunique = SqlStaticData.createUnique(tableName, columnName);
			    				...
			    				//System.out.println("一对多 添加外键的sql 测试："+testalterAddForeign);
								//System.out.println("一对多 添加外键的sql 成功："+alterAddForeign);
								...
								String bridgeWPK = SqlStaticData.createUnionPrimaryKey(joinTableName, colmun1, colmun2);
								
								//联合主键 添加外键的sql 测试：alter table c_s add constraint c_s_uni_on_pk primary key(course_id,course_id)
								//联合主键 添加外键的sql 成功：alter table c_s add(primary key(course_id,student_id))
								
								
								//System.out.println("联合主键 添加外键的sql 测试："+testbridgeWPK);
								//System.out.println("联合主键 添加外键的sql 成功："+bridgeWPK);
								...
								String fk_refenence_1 =SqlStaticData.createAddForeignKeySql(joinTableName, colmun1, referenceTableName1, joinColumn1.joinColumn());
								String fk_refenence_2 =SqlStaticData.createAddForeignKeySql(joinTableName, colmun2, referenceTableName2, joinColumn2.joinColumn());
								//String fk_refenence_1 ="alter table "+joinTableName+" add constraint "+joinTableName+"_"+colmun1+"_fk"+" foreign key("+colmun1+") references "+referenceTableName1+"("+joinColumn1.joinColumn()+") on delete cascade";
								//String fk_refenence_2 ="alter table "+joinTableName+" add constraint "+joinTableName+"_"+colmun2+"_fk"+" foreign key("+colmun2+") references "+referenceTableName2+"("+joinColumn2.joinColumn()+") on delete cascade";
								
								//System.out.println("多对多  添加外键 关系  测试 1 "+testfk_refenence_1);
								//System.out.println("多对多  添加外键 关系  成功 1 "+fk_refenence_1);
								//System.out.println("多对多  添加外键 关系  测试 2 "+testfk_refenence_2);
								//System.out.println("多对多  添加外键 关系  成功 2 "+fk_refenence_2);
								 //删除联合主键的sql
								 ...
								 if(append){
										String fksql = SqlStaticData.createTableConstraintSql(joinTableName);
									   // String fksql = "select CONSTRAINT_NAME from user_constraints e where CONSTRAINT_TYPE!='P' and upper(e.table_name) = '"+joinTableName.toUpperCase()+"'";
									    //System.out.println("多对多 查询外键的sql 测试："+testfksql);
										//System.out.println("多对多 查询外键的sql 成功："+fksql);
										ResultSet fkset = statement.executeQuery(fksql);
										while(fkset.next()){
											String constraint_name = fkset.getString("CONSTRAINT_NAME");
											String delete_fk_refenence = SqlStaticData.createDropConstraintSql(joinTableName, constraint_name);
											//System.out.println(fksql+"  查询到的外键关系 222 : " +constraint_name);
											//String delete_fk_refenence = "alter table "+joinTableName+"  drop constraint "+constraint_name;
											//System.out.println("多对多 删除外键的sql 测试："+testdelete_fk_refenence);
											//System.out.println("多对多 删除外键的sql 成功："+delete_fk_refenence);
	...
	public Map<String,String> getSqlByTables(List<Table> tables){
		...
		for(Table t:tables){
			...
			map.put(t.getTableName(), SqlStaticData.createTableSql(t.getTableName(), column));
		}
		return map;
	}
